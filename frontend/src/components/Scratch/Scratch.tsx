...
function getPreferredDiffTab(defaultDiffTab: DefaultDiffTab): TabId {
    return defaultDiffTab === DefaultDiffTab.OBJDIFF
        ? TabId.OBJDIFF
        : TabId.DIFF;
}
...
function applyDefaultDiffTab(
    ...
    const preferredTab = getPreferredDiffTab(defaultDiffTab);
...
export default function Scratch({
    ...
    if (container.width) {
        ...
        if (layoutName !== preferredLayout) {
            ...
            setLayout(
                applyDefaultDiffTab(
                    cloneLayout(DEFAULT_LAYOUTS[preferredLayout]),
                    defaultDiffTab,
                ),
            );
        }
    ...
    useEffect(() => {
        ...
        visitLayout(layout, (node) => {
            if (node.kind === "pane" && node.tabs.includes(preferredTab)) {
                if (node.activeTab !== preferredTab) {
                    needsUpdate = true;
                }
            }
        });
        ...
        setLayout((currentLayout) =>
            applyDefaultDiffTab(cloneLayout(currentLayout), defaultDiffTab),
        );
    }, [defaultDiffTab, layout]);
